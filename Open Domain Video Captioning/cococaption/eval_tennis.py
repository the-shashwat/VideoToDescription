...
if __name__ == '__main__':
    rng = range(2)
    act_file_path = "../Tennis/actual_captions.txt"
    res_file_path = "../Tennis/data/rgb-of-lstm-80-200/S2VT_results.txt"
    datasetGTS = {'annotations' : []}
    datasetRES = {'annotations' : []}
    with open(act_file_path, "r") as fd:
        content = fd.readlines()
        cnt = 0
        for line in content:
            pid, caption = line.split("\t")
            x = {}
            x['image_id'] = cnt
            x['caption'] = caption
            datasetGTS['annotations'].append(x)
            cnt += 1

    with open(res_file_path, "r") as fd:
        content = fd.readlines()
        cnt = 0
        for line in content:
            pid, caption = line.split("\t")
            ...
            x['image_id'] = cnt
            x['caption'] = caption
            datasetRES['annotations'].append(x)
            cnt += 1

...
#=====================================================================================
video_path = './data/points_data/points_with_captions.csv'
rgb_dir_train = './data/points_data/train_feats/'
flow_dir_train = './data/points_data/train_flow_feats2'
rgb_dir_test = './data/points_data/test_feats/'
flow_dir_test = './data/points_data/test_flow_feats2'

video_train_feat_path = './data/points-data/train-feats/'
video_test_feat_path = './data/points-data/test-feats/'

video_train_data_path = './data/commentary/sents_train.txt'
video_test_data_path = './data/commentary/sents_test.txt'

model_path = './rgb-of-lstm-80-100-models/'
...
def get_video_train_data(video_data_path):
    video_data = pd.read_csv(video_data_path, sep=',')
    video_data = video_data[video_data['set'] == 'train']
    return video_data

def get_video_val_data(video_data_path):
    video_data = pd.read_csv(video_data_path, sep=',')
    video_data = video_data[video_data['set'] == 'val']
    return video_data

def get_video_test_data(video_data_path):
    video_data = pd.read_csv(video_data_path, sep=',')
    video_data = video_data[video_data['set'] == 'test']
    return video_data
...
def train():
    train_data = get_video_train_data(video_path)
    train_captions = train_data['caption'].values
    val_data = get_video_val_data(video_path)
    ...
    np.save("./data/rgb-of-lstm-80-100/wordtoid", wordtoid)
    np.save("./data/rgb-of-lstm-80-100/idtoword", idtoword)
    np.save("./data/rgb-of-lstm-80-100/bias_init_vector", bias_init_vector)
...
#	loss_to_draw_epoch = []
        ...
        for start, end in zip(range(0, len(train_data), batch_size),
                              range(batch_size, len(train_data), batch_size)):

            start_time = time.time()
            current_batch = train_data[start:end]
            current_videos = current_batch['vid'].values
            current_rgb_feats = []
            current_flow_feats = []
            for vid in current_videos:
                current_rgb_feats.append(os.path.join(rgb_dir_train,vid))
                current_flow_feats.append(os.path.join(flow_dir_train,vid))
		...
		for v in current_videos:
            ...
            current_feats_vals = []
            for rgb_feat, flow_feat in zip(current_rgb_feats, current_flow_feats):
                f1 = np.load(rgb_feat)
                f2 = np.load(flow_feat)
                current_feats_vals.append(np.hstack((f1,f2)))
                #current_feats_vals.append(f1)
...
def test(model_path='./rgb-of-lstm-80-100-models/'):
    test_data = get_video_test_data(video_path)
    print test_data
    test_videos = test_data['vid'].unique()
    idtoword = pd.Series(np.load('./data/rgb-of-lstm-80-100/idtoword.npy').tolist())
    tf.reset_default_graph()
    bias_init_vector = np.load('./data/rgb-of-lstm-80-100/bias_init_vector.npy')
    model = V2D(
        dim_image=dim_image,
    ...
    sess = tf.InteractiveSession(config=config)
    sess.run(tf.global_variables_initializer())	
    saver.restore(sess, os.path.join(model_path, 'model-550'))
    #print(sess.run(model.Wemb))
    test_output_txt_fd = open('./data/rgb-of-lstm-80-100/S2VT_results.txt', 'w')
    test_actual_output_txt_fd = open('actual_captions.txt','w')
    for idx, row in test_data.iterrows():
	test_actual_output_txt_fd.write(str(row['pid'])+"\t")
    ...
    for idx, video_feat_path in enumerate(test_videos):
        ...
        rgb_feat = np.load(os.path.join(rgb_dir_test,video_feat_path))
        flow_feat = np.load(os.path.join(flow_dir_test,video_feat_path))
        video_feat = np.hstack((rgb_feat,flow_feat))[None, ...]
        #video_feat = video_flow_feat[None, ...]
        #video_feat = rgb_feat[None, ...]
	...
	generated_sentence = generated_sentence.replace(' <pad>', '')
        ...
        test_output_txt_fd.write(test_data.iloc[idx]["pid"] + '\t')
